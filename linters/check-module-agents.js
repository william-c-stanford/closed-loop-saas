...
function checkModuleAgents(repoRoot, config) {
  ...
  for (const srcDir of sourceDirs) {
    ...
    for (const mod of modules) {
      ...
      const claudeMdPath = path.join(modPath, 'CLAUDE.md');
      ...
      if (!fs.existsSync(claudeMdPath)) {
        if (sourceFileCount >= requiredAbove) {
          results.push({
            check: 'module-claude-md-required',
            status: 'warn',
            message: `${srcDir}/${mod.name}/ has ${sourceFileCount} files but no CLAUDE.md`,
            detail: `Run: /context-gardening:scaffold-module ${srcDir}/${mod.name}/`,
          });
        }
        continue;
      }
      ...
      if (missingSections.length > 0) {
        results.push({
          check: 'module-claude-md-structure',
          status: 'warn',
  ...
  if (results.length === 0) {
    results.push({ check: 'module-claude-md', status: 'ok', message: 'No source modules found to check' });
  }
